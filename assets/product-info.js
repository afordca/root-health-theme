	...
	customElements.define(
		...
		class ProductInfo extends HTMLElement {
			...
			initCheckoutRedirect() {
				// Check if this product-info element has the redirect-to-checkout attribute
				if (this.dataset.redirectToCheckout !== "true") return;

				console.log(
					"Root Health: Initializing checkout redirect for product-info"
				);

				// Find the product form within this element
				const productForm = this.querySelector("product-form");
				if (!productForm) {
					console.log("Root Health: No product-form found in product-info");
					return;
				}

				const form = productForm.querySelector(
					'form[data-type="add-to-cart-form"]'
				);
				if (!form) {
					console.log("Root Health: No add-to-cart form found");
					return;
				}
				...
				paymentButtons.forEach((button) => {
					button.addEventListener("click", (evt) => {
						console.log("Root Health: Payment button clicked!", button);
						console.log("Root Health: Button type:", button.type);
						console.log("Root Health: Button form:", button.form);
						console.log("Root Health: Form element:", form);

						// Check if button is disabled
						if (button.disabled) {
							console.log(
								"Root Health: Button is disabled, preventing submission"
							);
							return;
						}

						// If button doesn't have a form reference, manually trigger form submit
						if (!button.form || button.form !== form) {
							console.log(
								"Root Health: Button not properly linked to form, manually triggering submit"
							);
							evt.preventDefault();
							form.dispatchEvent(
				...
				console.log("Root Health: Adding form submit listener");

				// Add event listener to intercept form submission
				form.addEventListener("submit", (evt) => {
					console.log("Root Health: Form submit intercepted");
					console.log("Root Health: Form data:", new FormData(form));
					console.log("Root Health: Form action:", form.action);
					console.log("Root Health: Form method:", form.method);

					// Check if form has required variant ID
					const variantInput = form.querySelector('input[name="id"]');
					console.log("Root Health: Variant input:", variantInput);
					console.log("Root Health: Variant ID:", variantInput?.value);

					if (!variantInput?.value) {
						console.log(
							"Root Health: No variant selected, form submission may fail"
						);
					}

					// Let the original product-form handle the cart addition
					// But listen for the cartUpdate event to redirect
					// const unsubscribe = subscribe(PUB_SUB_EVENTS.cartUpdate, (event) => {
					// 	console.log("Root Health: Cart updated, redirecting to checkout");
					// 	console.log("Root Health: Cart update event:", event);
					...
					// Also listen for cart errors
					const unsubscribeError = subscribe(
						PUB_SUB_EVENTS.cartError,
						(event) => {
							console.log("Root Health: Cart error occurred:", event);
							unsubscribeError(); // Clean up the subscription
						}
					);
				});
			}
